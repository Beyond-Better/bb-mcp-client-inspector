...
describe('Trigger Error Tool', () => {
  ...
  it('should trigger validation error', async () => {
    ...
    assertEquals(
      (result.content[0] as any).text.includes('Validation Error'),
      true,
    );
    assertEquals(
      (result.content[0] as any).text.includes('Invalid input'),
      true,
    );
  });
  ...
  it('should use default message when not provided', async () => {
    ...
    assertEquals(result.isError, true);
    assertEquals(
      (result.content[0] as any).text.includes('Triggered validation error'),
      true,
    );
  });
